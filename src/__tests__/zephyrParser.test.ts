import { zephyrTableToGraphQLParser } from "../utils/parsers";
...
test("it should throw an error for an invalid ZephyrTableOriginal address", () => {
  ...
  expect(() => zephyrTableToGraphQLParser(data)).toThrowError(
    "Invalid ZephyrTableOriginal address: invalid_address",
  );
});
